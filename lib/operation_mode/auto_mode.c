...
int front_hard_lim = FRONT_HARD_LIM; // hard limit - dont go forward
int front_soft_lim = FRONT_SOFT_LIM; // soft limit - begin wide turn
...
void mode_5() {
  ...
  if (distance_values[1] < front_hard_lim) {
    ...
    if (distance_values[1] < front_soft_lim) {
      front_soft_lim = FRONT_SOFT_LIM;
      sprintf(msg, "\nsoft %d", (distance_values[0] < distance_values[2]));
      serial0_print_string(msg);
      motor_softturn_forward(distance_values[0] < distance_values[2]);
    } else {
...
void mode_7() {
  ...
  if ( centre < front_hard_lim ) {
    ...
    if (left < side_hard_lim || right < side_hard_lim) {
      ...
      motor_hardturn_forward(turn_bias_manager(left, right));
      ...
      motor_softturn_forward(turn_bias_manager(left, right));
